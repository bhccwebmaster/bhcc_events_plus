 ...
 *
 * @FieldWidget(
 *   id = "bhcc_events_plus_facet_checkbox",
 *   module = "bhcc_events_plus",
 *   label = @Translation("Event facets"),
 *   field_types = {
 *     "entity_reference"
 *   },
...
class FacetFieldWidget extends OptionsWidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $enabled = [];
    if ($user_input = $form_state->getValue('bhcc_event_channel')) {
      ...
      foreach ($items->getEntity()->bhcc_event_channel as $channel) {
    ...
    if (empty($options)) {
      $element['#description'] = $this->t('Select event channels to add facets');
    }
